...
sed -i "s/192.168.1.1/192.168.3.7/" package/base-files/files/bin/config_generate
...
sed -i "s/DEFAULT_PACKAGES:=/DEFAULT_PACKAGES:=luci-app-firewall luci-app-package-manager luci-app-upnp luci-base luci-ssl luci-theme-bootstrap \
coremark wget-ssl curl htop nano zram-swap kmod-lib-zstd kmod-tcp-bbr bash openssh-sftp-server block-mount resolveip /" include/target.mk
